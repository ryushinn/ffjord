...
def make_layer(dim_in, dim_out, stride):
    if stride is None:
        layer_kwargs = {}
    elif stride == 1:
    ...
    return ConcatConv2d(dim_in, dim_out, **layer_kwargs)
...
class ODENet(nn.Module):
    ...
    def __init__(self, hidden_dims, input_shape, strides, nonlinearity="softplus"):
        ...
        for dim_out, stride in zip(hidden_dims + (input_shape[0],), strides):
            layers.append(make_layer(hidden_dims[0], dim_out, stride))
...
class ODEUNet(nn.Module):
    def __init__(
        ...
        layers = []
        dims = dims + list(reversed(dims)) + [init_dim, ]
        strides = strides + [1, ] + \
            [s if s==1 else -s for s in reversed(strides)]
        
        dim_in = init_dim
        for dim_out, stride in zip(dims, strides):
            layers.append(make_layer(dim_in, dim_out, stride))
            dim_in = dim_out
            if stride == -2:
                dim_in *= 2
    ...
    def forward(self, t, x):
        ...
        for i, layer in enumerate(self.layers):
            if self.strides[i] == 2:
                left_concat.append(x)
            
            x = layer(t, x)
            x = self.activation(x)

            if self.strides[i] == -2:
                left = left_concat.pop(-1)
                x = torch.concatenate([x, left], dim=1)
